...
pub trait Vault {
    ...
    fn accept_funds(&self) {
        ...
        self.accept_funds_event(&self.call_value().moa_value(), &dcdt_transfers_multi);
    ...
    fn accept_funds_echo_payment(
        ...
        let dcdt_transfers_multi = self.dcdt_transfers_multi();
        self.accept_funds_event(&moa_value, &dcdt_transfers_multi);

        self.call_counts(ManagedBuffer::from(b"accept_funds_echo_payment"))
            .update(|c| *c += 1);

        (moa_value.clone_value(), dcdt_transfers_multi).into()
    }
    ...
    fn reject_funds(&self) {
        let dcdt_transfers_multi = self.dcdt_transfers_multi();
        self.reject_funds_event(&self.call_value().moa_value(), &dcdt_transfers_multi);
